...
if ($mysqli->multi_query("SELECT SectorName, AVG((100*(TRADES.ClosePrice - TRADES.OpenPrice))/TRADES.OpenPrice) AS percent_gain FROM FORECAST INNER JOIN TRADES ON TRADES.dateID = FORECAST.dateID INNER JOIN DATES AS D ON D.ID = TRADES.dateID INNER JOIN SECURITIES AS SEC ON SEC.symbol = TRADES.symbol INNER JOIN SECTOR ON SECTOR.ID = SEC.SectorID WHERE D.MM = ".$MM." AND ".$RAIN." AND FORECAST.MaxTemp > ".$TEMP." AND FORECAST.MinTemp < ".$TEMP." GROUP BY SectorName ORDER BY percent_gain DESC;")) {

...
if ($mysqli->multi_query("SELECT SectorName ,AVG((100*(T.ClosePrice-T.OpenPrice)/T.OpenPrice)) AS GAIN, FLOOR((MaxTemp+MinTemp)/20)*10 AS TempBracket FROM TRADES AS T INNER JOIN FORECAST AS F ON T.dateID = F.dateID INNER JOIN SECURITIES AS S ON S.symbol = T.symbol INNER JOIN SECTOR ON SECTOR.ID = S.SectorID GROUP BY SectorName, TempBracket ORDER BY GAIN DESC;")) {

...
if ($mysqli->multi_query("SELECT AVG((100*(T.ClosePrice-T.OpenPrice)/T.OpenPrice)) AS GAIN, FLOOR((MaxTemp+MinTemp)/20)*10 AS TempBracket FROM TRADES AS T INNER JOIN FORECAST AS F ON T.dateID = F.dateID WHERE T.symbol = '".$SYMBOL."' GROUP BY TempBracket ORDER BY GAIN DESC;")) {

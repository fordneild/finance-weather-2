...
if ($mysqli->multi_query("SELECT TRADES.symbol, AVG((100*(TRADES.ClosePrice - TRADES.OpenPrice))/TRADES.OpenPrice) AS percent_gain FROM FORECAST INNER JOIN TRADES ON TRADES.dateID = FORECAST.dateID INNER JOIN DATES AS D ON D.ID = TRADES.dateID WHERE D.MM = ".$MM." AND ".$RAIN." AND FORECAST.MaxTemp > ".$TEMP." AND FORECAST.MinTemp < ".$TEMP." GROUP BY symbol ORDER BY percent_gain DESC;")) {

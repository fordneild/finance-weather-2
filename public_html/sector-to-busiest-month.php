...
if ($mysqli->multi_query("SELECT SectorName, MM, AVG(VOLUME) AS V FROM TRADES AS T INNER JOIN DATES AS D ON T.DateID = D.ID INNER JOIN SECURITIES AS SEC ON SEC.symbol = T.symbol INNER JOIN SECTOR ON SECTOR.ID = SEC.SectorID GROUP BY SECTOR.SectorName, MM ORDER BY V DESC;")) {
